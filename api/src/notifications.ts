...
export async function saveSubscription(
  ...
  if (
    ...
    throw new Error("Onbekende pushdienst.");
  if (
    !body.keys?.p256dh ||
    !body.keys.auth ||
  ...
  try {
    ...
    for (const event of events) {
      ...
      const occurrences = generateOccurrences(
        ...
        new Date(+now + ahead - 60000),
        new Date(+now + ahead + 60000),
        event.recurrenceTimezone || event.calendar.timezone,
        event.excludedDates,
      );
        ...
        for (const member of event.calendar.members)
          for (const subscription of member.user.subscriptions) {
            ...
            const claimed = await prisma.notificationDelivery.createMany({
              data: [{ subscriptionId: subscription.id, occurrenceKey }],
              skipDuplicates: true,
            });
            ...
            try {
              await webpush.sendNotification(
                ...
                JSON.stringify({
                  title: event.title,
                  body: `${event.calendar.name} · ${occurrence.start.toLocaleTimeString("nl-NL", { timeZone: event.calendar.timezone, hour: "2-digit", minute: "2-digit" })}${event.location ? " · " + event.location : ""}`,
                  tag: occurrenceKey,
                }),
              ...
              if (status === 404 || status === 410)
                ...
                await prisma.notificationDelivery.deleteMany({
                  where: { subscriptionId: subscription.id, occurrenceKey },
                });
            }
          }
      }
